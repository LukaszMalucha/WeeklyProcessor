...
def unique_product_id(filename):    
    ...
    # YORK FIX
    dataset['product_name'] = np.where((dataset['brand'] == "YORK") & (dataset['product_name'].str.contains("064-48208-000")), "YK", dataset['product_name'])
    dataset['product_name'] = np.where((dataset['brand'] == "YORK") & (dataset['product_name'].str.contains("064-48207-000")), "YK", dataset['product_name'])
...
#    product_id_unique = list(dataset['product_identifier'].unique())
    ...
    dataset.to_csv('7_documents_unique_product_id.csv',  encoding='utf-8-sig', index=False)
