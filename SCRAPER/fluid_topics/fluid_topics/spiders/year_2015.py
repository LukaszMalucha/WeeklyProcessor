...
# Helper functions to create a list of dates for 2015 - So far it's Feb
def create_annual_list_2015():
	annual_list = []
	for m in range(1,10):
		for d in range(1, 10):
			annual_list.append(f'2015-0{m}-0{d}_2015-0{m}-0{d}')
		for d in range(10, 32):
			annual_list.append(f'2015-0{m}-{d}_2015-0{m}-{d}')   
			
	for m in range(10,13):
		for d in range(1, 10):
			annual_list.append(f'2015-{m}-0{d}_2015-{m}-0{d}')
		for d in range(10, 32):
			annual_list.append(f'2015-{m}-{d}_2015-{m}-{d}')    
...
class Year2015Spider(scrapy.Spider):
	name = 'year_2015'
	allowed_domains = ['johnsoncontrols.fluidtopics.net']
	start_urls = ['http://johnsoncontrols.fluidtopics.net/']

	annual_list_2015 = (create_annual_list_2015())

	# Year 2015 has to be scraped monthly
	def parse(self, response):
		...
		for date in self.annual_list_2015:
			self.driver.get(
