...
def create_annual_list_2020():
	...
	for i in range(3, 4):
		for d in range(10, 32):
			annual_list.append(f'2020-0{i}-{d}_2020-0{i}-{d}')
...
class Year2019Spider(scrapy.Spider):
	...
	annual_list = (create_annual_list_2020())
	...
	def parse(self, response):
		...
		self.driver.get('http://johnsoncontrols.fluidtopics.net/')   #  lukasz.malucha@jci.com
		sleep(20)

		for date in self.annual_list:
			self.driver.get(
				f'https://johnsoncontrols-uat.fluidtopics.net/search/all?filters=brand~%2522FRICK%2522&period=custom_{date}&content-lang=en-US')
			sleep(6)
